...
following tutorial uses [ngsutilsj fastq-filter](/projects/ngsutilsj/) to process and filter FASTQ
files.

## Table of contents

- [Sequence QC tools](#sequence-qc-tools)
- [DNA-seq filtering methods](#dna-seq-filtering-methods)
- [RNA-seq filtering methods](#rna-seq-filtering-methods)
...
can calculate QC metrics for FASTQ data that is streamed as part of a stdin/stdout pipeline. [ngsutilsj fastq-stats](/projects/ngsutilsj/)
supports this method and calculates per-base sequence quality, GC %, per-base call
frequency, and adapter content. Additionally, [ngsutilsj fastq-stats](/projects/ngsutilsj/) also supports interleaved FASTQ
files and reporting first/second read stats separately. The final report from [ngsutilsj fastq-stats](/projects/ngsutilsj/) is
a raw text file that requires further processing to produce figures.

Importantly, when inserted into a FASTQ pipeline, [ngsutilsj fastq-stats](/projects/ngsutilsj/) requires no extra
...
Here are some of the ways to cleanup FASTQ reads with [ngsutilsj fastq-filter](/projects/ngsutilsj/):
...
1. Limit ambiguous/wildcard (N) bases
...
<br/>
Finally, we can add [ngsutilsj fastq-stats](/projects/ngsutilsj/) into the mix to calculate some
summary statistics on the fly for both the raw FASTQ reads and the post-filtering reads.
By merging the paired FASTQ files and using [ngsutilsj fastq-stats](/projects/ngsutilsj/),
...
<br/>
As shown above and following *nix conventions, many of the [ngsutilsj](/projects/ngsutilsj/) tools can
operate in streaming mode by specifying "`-`" as the input filename, reading input from *stdin*. [ngsutilsj fastq-stats](/projects/ngsutilsj/)
